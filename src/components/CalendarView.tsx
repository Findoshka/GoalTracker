...
type CalTask =
  | { kind: 'inbox'; id: string; title: string; color: string; completed: boolean }
  | { kind: 'goal'; goalId: string; stageId: string; weekId: string; taskId: string; title: string; color: string; completed: boolean };

export function CalendarView() {
  const { inbox, goals, addInboxTask, toggleInboxTask, toggleTask } = useStore();
  ...
  const tasksByDate = useMemo(() => {
    ...
    inbox.forEach(t => {
      if (t.dueDate) {
        ...
        map[t.dueDate].push({ kind: 'inbox', id: t.id, title: t.title, color: '#a78bfa', completed: t.completed });
      }
    });
    goals.forEach(g => {
      ...
      g.monthlyStages.forEach(ms => ms.weeklyGoals.forEach(wg => wg.tasks.forEach(t => {
        if (t.dueDate) {
          if (!map[t.dueDate]) map[t.dueDate] = [];
          map[t.dueDate].push({ kind: 'goal', goalId: g.id, stageId: ms.id, weekId: wg.id, taskId: t.id, title: t.title, color: c, completed: t.completed });
        }
      })));
    });
    return map;
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden" style={{ background:'#fdf6fa' }}>
              ...
              return (
                <div key={cell.date}
                  ...
                  style={{
                    ...
                    background: isSelected ? 'rgba(249,168,212,.13)' : isToday ? 'rgba(249,168,212,.08)' : isWeekend && cell.isCurrentMonth ? 'rgba(249,168,212,.03)' : undefined,
                  }}
                  onClick={() => {
                    if (!cell.isCurrentMonth) return;
                    // On mobile: select day to show panel; on desktop: open add input
                  ...
                  {/* Tasks — hidden on mobile (shown in panel below) */}
                  <div className="flex-1 overflow-hidden space-y-[3px] hidden md:block">
                    {tasks.slice(0, 5).map((t, ti) => (
                      <div key={ti}
                        ...
                        <button
                          className="shrink-0 w-[14px] h-[14px] rounded-[3px] flex items-center justify-center ml-1 transition-all duration-150"
                          style={{
      ...
      {/* Mobile day panel — shown when a day is selected */}
      {selectedDay && (
